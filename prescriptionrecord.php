...
<div class="container">
						...
						<td width="66%">
							<select name="medicineid" id="medicineid" onchange="loadmedicine(this.value)">
								...
								while ($rsmedicine = mysqli_fetch_array($qsqlmedicine)) {
									echo "<option value='$rsmedicine[medicineid]'>$rsmedicine[medicinename] ( RM $rsmedicine[medicinecost] )</option>";
								}
								?>
							</select>
						</td>
	...
	<table class="table">
		<tbody>
          ...
          <td>RM $rs[cost]</td>
		   <td>$rs[unit]</td>
		   <td  align='right'>RM" . $rs['cost'] * $rs['unit'] . "</td>";
				if (!isset($_SESSION['patientid'])) {
			...
			<tr>
				...
				<th align="right">RM<?php echo $gtotal; ?></th>
				<td></td>
			</tr>
			<tr>
				<td colspan="6">
